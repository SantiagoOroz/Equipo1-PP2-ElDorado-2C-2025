...
# RUTA a tu modelo Llama descargado - AJUSTA ESTA RUTA A TU ARCHIVO .gguf
LLAMA_MODEL_PATH = r"C:\Users\capod\.lmstudio\models\lmstudio-community\Meta-Llama-3-8B-Instruct-GGUF\Meta-Llama-3-8B-Instruct-Q4_K_M.gguf"

# 🚀 AJUSTES DE VELOCIDAD PARA HARDWARE CON GPU PEQUEÑA
N_GPU_LAYERS = 0      # Forzar el uso de la CPU y RAM (más eficiente para un 8B con 2GB VRAM)
N_BATCH = 1024        # Tamaño de lote (valor alto está bien para CPU/RAM)
N_THREADS = 12        # Hilos/núcleos de CPU a usar (ajusta según tus 8 núcleos físicos/16 lógicos)
...
def inicializar_rag_chain():
    ...
    try:
        ...
        llm = LlamaCpp(
            ...
            n_batch=N_BATCH,
            # 💡 Añadido para usar múltiples núcleos de la CPU
            n_threads=N_THREADS, 
            verbose=False,
        )
        ...
        qa_chain = RetrievalQA.from_chain_type(
            llm=llm,
            chain_type="stuff",
            # 💡 Reducido a k=2 para un contexto más pequeño y un procesamiento de 'stuff' más rápido
