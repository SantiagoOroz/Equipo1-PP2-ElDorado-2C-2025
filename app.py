...
@metrics.track_db_operation("rag_context")
...
def get_metrics():
    """Endpoint para obtener las métricas del sistema y la aplicación."""
    try:
...
""".strip()
    ...
    # Agregar métricas específicas del request
    response_data["metrics"] = {
        "processing_time": round(latency, 3),
        "context_size": len(combined_context),
        "response_size": len(llama_reply)
    }
...
@metrics.track_endpoint("transcribe")
def api_transcribe():
    """
...
@metrics.track_endpoint("synthesize")
def api_synthesize():
    """
...
if __name__ == "__main__":
    ...
    print("Iniciando con métricas activadas...")
    try:
